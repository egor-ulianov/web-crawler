...
export class CrawlPlannerService {
  ...
  public async crawlWebsites(): Promise<void> {
    ...
    const websitesToCrawl: Array<WebsiteCrawlExecutionPlan> =
      await this.getActiveWebsiteRecordsWithExpiredLastExecutionDate();
  ...
  > {
    ...
    return entities.map((entity) =>
      WebsiteCrawlExecutionPlan.fromEntity(entity),
    );
  }
  ...
  public async planWebsiteCrawlExecution(): Promise<void> {
    ...
    websites.forEach(async (website) => {
      const currentlyRunningPendingForWebsite: number =
        await this._websiteCrawlExecutionPlanRepository.count({
      ...
      console.log(
        `Currently running/pending for website ${website.siteId}: ${currentlyRunningPendingForWebsite}`,
      );
      ...
      if (website.date == null) {
        const entity: WebsiteCrawlExecutionPlanEntity =
          new WebsiteCrawlExecutionPlanEntity();
      ...
      if (new Date(website.date).getTime() > currentDate.getTime()) return;

      let nextExecutionDate: Date = new Date(website.date);
      nextExecutionDate = new Date(
        nextExecutionDate.getTime() + website.periodicity * 1000,
      );
